...
import { GaugeOptions, Gauge } from './gauge.native';
import { GaugeDefaults } from './gauge-defaults.service';

@Component({
  selector: 'app-mwl-gauge',
  template: ''
})
export class GaugeComponent implements AfterViewInit, OnChanges, GaugeOptions {
  ...
  ngAfterViewInit(): void {
    ...
    this.gauge = new Gauge(this.elm.nativeElement, options);
  ...
  private updateMax(): void {
    if (this.gauge) {
      ...
      // this forces a repaint of the gauge
      this.gauge.setValue(this.value);
    }
  }
}
